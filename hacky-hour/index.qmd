...
If you would like to subscribe to the calendar click [here](https://calendar.google.com/calendar?cid=dDB1bW91dmFvbzNoNjJrY2Zjajd2Y2ZjazhAZ3JvdXAuY2FsZW5kYXIuZ29vZ2xlLmNvbQ) or head on over to the Events tab of this website to find out more.

### Date & Time

Hacky hour takes place on Thursdays from 11:00 AM to 12:30 PM, with a virtual option available via [Zoom](https://monash.zoom.us/j/6480938551?pwd=sEeL1EM5ydNEWWcFESvUYj3iNDM4Ci.1). 

### Location

Room 317, Building 6, 29 Ancora Imparo Way (check Current Schedule (@sec-current-schedule) to see where it's happening this week). Except for the second week (TBC)
...
#| echo: false
#| output: true
ggiraph::girafe(ggobj = render_calendar(here::here("hacky-hour/schedules/2025_S2_schedule.yaml")))
```
...
#| echo: false 
#| output: true 
read_yaml(here::here("hacky-hour/schedules/2025_S2_schedule.yaml")) |>
  yaml_date_nodes_to_tbl() |>
  tidy_date_tbl() |>
  verify_date_df() |>
  dplyr::mutate(id = row_number()) |>
